...
|`--compat`
|target XP version for the API call. Value must be in the form `X` or `X.Y` (digits only, e.g. `7` or `7.16`). Use a value starting with `7` (such as `7` or `7.16`) to target XP 7. Default targets XP 8.
|===
...
|`--archive`
|outputs dump output to an archive (%name%.zip) file (default is false). Only effective in compat mode (`--compat 7`).
...
|`--archive`
|loads dump from an archive (%name%.zip) file (default is false). Only effective in compat mode (`--compat 7`).
...
OPTIONS:
   ...
   --skip-ids              Flag to skip ids in data when exporting. Only effective in compat mode (XP 7).
   --skip-versions         Flag to skip versions in data when exporting. Only effective in compat mode (XP 7).
   ...
   --compat value          XP version compatibility mode. Value must be in the form "X" or "X.Y" (e.g. "7" or "7.16"). Use a value starting with "7" to use the legacy XP 7 API format. Default uses the XP 8 API format.
----

NOTE: The `--skip-ids`, `--skip-versions` and `--dry` options are not supported by the XP 8 export API and are only sent to the server in compat mode (`--compat 7`).
...
NOTE: The `--dry` option is not supported by the XP 8 import API and is only sent to the server in compat mode (`--compat 7`).
