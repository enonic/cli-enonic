...
|Path to use for the relative home directory for the CLI (instead of the default `$HOME`, or the root directory of your system user). For example, by setting this to `/tmp` you'd have CLI create a folder called `/tmp/.enonic`, where the sandboxes and distros will live.
...
WARNING: For a clustered installation, the snapshot-location must be on a shared file-system.
 ...
 $ enonic snapshot create [-r <value>] [-a <value>] [--cred-file <value>] [-f] [--compat <value>]
...
|`--compat`
|XP version compatibility mode. Set to `"7"` (or any value starting with `"7"`) to use the legacy XP 7 API format. Default uses the XP 8 API format.
|===
...
Restore a named snapshot. See https://developer.enonic.com/docs/xp/stable/deployment/backup-restore#snapshot-restore[Backup and Restore] for more information on snapshots.
 ...
 $ enonic snapshot restore [--snap <value>] [--repo <value>] [-a <value>] [-f] [--compat <value>]
...
|`--compat`
|XP version compatibility mode. Set to `"7"` (or any value starting with `"7"`) to use the legacy XP 7 API format (synchronous response). Default uses the XP 8 API format (task-based, tracked via the `taskId` returned by the server).
|===
...
Export data from every repository. The result will be stored in the `$XP_HOME/data/dump` directory.
 ...
 $ enonic dump create [-d <value>] [--skip-versions <value>] [--max-version-age <value>] [--max-versions <value>] [-a <value>] [--cred-file <value>] [-f] [--compat <value>]
...
|`--archive`
|outputs dump output to an archive (%name%.zip) file (default is false). Only effective in compat mode (XP 7); in XP 8 the dump is always created as an archive.
...
|`--compat`
|XP version compatibility mode. Set to `"7"` (or any value starting with `"7"`) to use the legacy XP 7 API format (sends the `archive` parameter and uses a progress bar). Default uses the XP 8 API format.
|===
...
WARNING: A load will delete all existing repositories before loading the repositories present in the system-dump
 ...
 $ enonic dump load [-d <value>] [--upgrade] [--archive] [-a <value>] [--cred-file <value>] [-f] [--compat <value>]
...
|`--archive`
|loads dump from an archive (%name%.zip) file (default is false). Only effective in compat mode (XP 7); in XP 8 the archive is auto-detected.
...
|`--compat`
|XP version compatibility mode. Set to `"7"` (or any value starting with `"7"`) to use the legacy XP 7 API format (sends the `archive` parameter when the dump name ends in `.zip` and uses a progress bar). Default uses the XP 8 API format.
|===
...
|`--blob, -b`
|Triggers `BinaryBlobVacuumTask`, `NodeBlobVacuumTask`, `SegmentVacuumTask` and `VersionTableVacuumTask`. Without this flag, XP will choose the default vacuum tasks.
