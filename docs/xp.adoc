...
|Path to use for the relative home directory for XP ( Default is `$HOME`, or the user directory of your user). For example, by setting this to `/tmp` you'd have CLI create a folder called `/tmp/.enonic` and your sandboxes and similar will live there.
