...
CLI acts as an interface to a running enonic XP instance. You can customize connection settings with following environmental variables:
...
WARNING: A load will delete all existing repositories before loading the repositories present in the system-dump
 ...
 $ enonic dump load [--d <value>] [-y] [--upgrade] [--a <value>] [--na <value>]
