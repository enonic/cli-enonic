...
With Homebrew installed, run the following command in your terminal app to add enonic to repositories known by Homebrew:

 brew tap enonic/cli

. Now you can install CLI by running:

 brew install --no-quarantine enonic

TIP: When first running `enonic`, macOS may show a warning that it cannot verify the software.                                                                                                     

. Click **"Done"** (not "Move to Bin")                                                                                                                                                           
. Run this command to remove the quarantine:                                                                                                                                                     

 xattr -d com.apple.quarantine $(which enonic)                                                                                                                                                  

. Run enonic again - it should work now                                                                                                                                                          
                                                                                                                                                                                                   
Alternatively: System Settings → Privacy & Security, scroll down and click "Allow Anyway" for the enonic app.
