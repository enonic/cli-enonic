...
. Packs the contents of the `enonic/` folder into the root of `build/libs/<name>-0.0.0.jar` (`enonic/enonic.yaml` becomes `/enonic.yaml`, `enonic/cms/...` becomes `/cms/...`). Files outside `enonic/` are not included.
. Generates `META-INF/MANIFEST.MF` with `Bundle-SymbolicName` set to the application name, `Bundle-Version: 0.0.0`, `X-System-Version: [8.1,9)` and `Bundle-Name`/`X-Vendor-Name`/`X-Vendor-Url`/`X-Application-Url` taken from the descriptor.
