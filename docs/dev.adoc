...
.Example creating 'myProject' project in 'myProject' folder and setting version to '1.0.0-SNAPSHOT':
----
...
----
$ enonic project create myProject -d myProject -v 1.0.0-SHAPSHOT
----

The only values you will need to choose in the above examples are the starter repo and the sandbox name to link to.
If you need to automate this command completely for pipeline integration, you can use `enonic project create`:

.Example creating 'myProject' project non-interactively in 'myFolder' folder using the 'starter-vanilla' starter and 'Sandbox1' sandbox and setting version to '1.0.0-SNAPSHOT':
----
$ enonic project create -f -n myProject -d myFolder -r starter-vanilla --sb Sandbox1 -v 1.0.0-SHAPSHOT
----
