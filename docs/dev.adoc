...
NOTE: <<schema-applications, Schema applications>> have no Gradle build and cannot be deployed. Use `enonic project install` instead.
...
|`--skip-start`
|don't ask to start the sandbox linked to a <<schema-applications, schema application>> before installing

|`-f, --force`
|accept default answers to all prompts and run non-interactively
|===

IMPORTANT: Enonic XP instance must be running when executing this command! The exception is a <<schema-applications, schema application>> with a linked sandbox: CLI offers to start the sandbox first.
...
A *schema application* provides only schemas (content types, mixins, parts, etc.) and has no code and no Gradle build.
A project is treated as a schema application when it has an `enonic.yaml` (or `enonic.yml`) application descriptor in the project root and no Gradle wrapper (`gradlew`):
...
└── cms/
    ...
    └── content-types/
        └── article/
            └── article.yaml
----
<1> Application descriptor. `name` is required and must be a valid application name: segments of letters, digits and underscores separated by periods, max 63 characters (e.g. `com.example.myapp`).
<2> Application icon (optional).
<3> CMS descriptor, required. An application shipping `cms/cms.yaml` (or `cms/cms.yml`) is treated by XP as the owner of its schema.
...
description: "Schema Application Starter"
vendorName: "Enonic AS"
vendorUrl: "https://enonic.com"
----

Only the fields accepted by XP are allowed in the descriptor (`kind`, `name`, `title`, `description`, `vendorName`, `vendorUrl`, `url`, `config`); unknown fields are reported as an error.

`enonic project install` for a schema application:

. Checks the project: `name` in `enonic.yaml` must be present and valid, and `cms/cms.yaml` (or `cms/cms.yml`) must exist. Otherwise the command fails with an error; nothing is prompted or generated.
. Packs `enonic.yaml` (or `enonic.yml`), `enonic.svg` (when present) and the `cms/` folder into the root of `build/libs/<short name>.jar`, where the short name is the last segment of the application name (e.g. `myapp.jar` for `com.example.myapp`). Only `.yaml`, `.yml` and `.svg` files are packaged: from `cms/` other files (e.g. `.gitkeep`, `README.md`, `.png`) are skipped while the folder structure is kept. No other files of the project are included.
. Generates `META-INF/MANIFEST.MF` with `Bundle-SymbolicName` set to the `name` from the descriptor (XP requires them to match), `X-System-Version: [8.1,9)` and `Bundle-Name`/`X-Vendor-Name`/`X-Vendor-Url`/`X-Application-Url` taken from the descriptor. Schema applications have no version, so `Bundle-Version` is not written (OSGi and XP default to `0.0.0`).
. Installs the jar over HTTP. If a sandbox is linked to the project, it is started in detached mode first (unless `--skip-start` is given or it is already running) and CLI waits for XP to become ready. Without a linked sandbox the application is installed to the active remote (see `enonic remote`; by default `http://localhost:4848`), so any running XP instance can be used — a sandbox is not required for schema applications. Use `enonic project sandbox <name>` to link one.

`enonic project create` adjusts its wizard for schema applications: the application name given to the wizard is written to `enonic.yaml` as `name` (instead of `gradle.properties`), the project version prompt is skipped (schema applications are always installed with version `0.0.0`, the `--version` parameter is ignored), and the sandbox selection offers a `Don't use a sandbox` option, so the project can be created without one. Since the project type is only known after the repository is cloned, the version prompt for Gradle based projects is shown after the `Initializing project...` step.

NOTE: Gradle based commands (`build`, `clean`, `test`, `gradle`, `dev`, `deploy`) are not available for schema applications.

.Example creating and installing a schema application:
----
$ enonic project create -r enonic/starter-static-app -n com.example.myschemas -d myschemas
$ cd myschemas
$ enonic project install
----
...
Each sandbox will be associated with either a downloaded distribution (version) of Enonic XP or a Docker image, and will have its own Enonic XP home folder.
...
[[sandbox-docker]]
.Distribution or Docker image
****
A sandbox can run XP from either of two sources:

* An XP **distribution** archive (default) — the CLI downloads it from enonic.com and unpacks it into the sandbox folder.
* A **Docker image** (e.g. `enonic/xp:latest-sdk`) — the CLI runs `docker run` against the selected image and mounts the sandbox home folder into the container.

When creating a sandbox interactively, the CLI asks which source to use (Docker is offered only when `docker` is available on `$PATH`). In non-interactive mode (`-f`), the source is decided by the flags: `--version` selects a distribution, `--image` selects a Docker image. The two flags are mutually exclusive.

Docker-backed sandboxes use a container named `enonic-sandbox-<sandbox>`, map the http/management/info ports to the host, and bind-mount the sandbox home folder at `/enonic-xp/home` inside the container.
****
...
Starts a new sandbox wizard:
 ...
 $ enonic sandbox create [name] [-v <value> | -i <value>] [-a]
...
|`-v, --version`
|use specific distro version (mutually exclusive with `--image`)

|`-i, --image`
|use specific Docker image, e.g. `enonic/xp:7.13.4-sdk` (mutually exclusive with `--version`). See <<sandbox-docker>>.
...
NOTE: When `--image` is used, the CLI will pull the image (if not already present locally) before creating the sandbox. Docker must be installed and reachable on the host.
...
.Example creating sandbox called 'myBox' backed by the `enonic/xp:latest-sdk` Docker image
----
$ enonic sandbox create myBox -i enonic/xp:latest-sdk -f
----
...
* Sandbox1 ( windows-sdk-7.2.1 )
  ...
  Sandbox4 ( docker:enonic/xp:latest-sdk )
----
NOTE: Asterisk marks currently running sandbox. Entries whose source is shown as `docker:<image>` are Docker-backed sandboxes.
...
Upgrades the Enonic XP distribution (or switches the Docker image) used by a sandbox:

 $ enonic sandbox upgrade [name] [-v <value> | -i <value>]
...
|`-v, --version`
|enonic distribution version. Only valid for distribution-backed sandboxes.

|`-i, --image`
|new Docker image to use, e.g. `enonic/xp:7.13.4-sdk`. Only valid for Docker-backed sandboxes. See <<sandbox-docker>>.
...
NOTE: Downgrades are not permitted for distribution-backed sandboxes. The flag that applies depends on how the sandbox was created: `--version` for distribution-backed sandboxes, `--image` for Docker-backed sandboxes. Using the wrong flag is an error.
...
.Example switching Docker-backed sandbox 'myBox' to a different image:
----
$ enonic sandbox upgrade myBox -i enonic/xp:7.14.0-sdk
----
