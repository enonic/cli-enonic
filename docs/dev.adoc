...
. Packs `enonic.yaml` (or `enonic.yml`), `enonic.svg` (when present) and the `cms/` folder into the root of `build/libs/<short name>.jar`, where the short name is the last segment of the application name (e.g. `myapp.jar` for `com.example.myapp`). No other files of the project are included.
