...
* `sandbox start --prod` starts the sandbox in dev mode on XP 8.1 and later. The run mode is now passed explicitly to the XP launcher.

=== Improvements

* The obsolete hidden `--dev` flag has been removed from `create`, `project create`, `project deploy`, `sandbox create` and `sandbox start`. Sandboxes start in dev mode by default; use `--prod` for non-development mode.
