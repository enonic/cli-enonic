...
func ensureDirStructure() {
	...
	fmt.Fprintln(os.Stderr, "Ensure dir structure")
	...
	if util.GetCurrentOs() == "linux" {
		if snapCommon, snapExists := os.LookupEnv(common.SNAP_ENV_VAR); snapExists {
			...
			fmt.Fprintln(os.Stderr, "Snapcraft exists")
			...
			if enonicPathExists {
				...
				fmt.Fprintln(os.Stderr, "Enonic path does not exist")
				createFolderIfNotExist(snapPath)
			}
			...
			fmt.Fprintln(os.Stderr, "Snapcraft does not exist")
