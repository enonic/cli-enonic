...
var Deploy = cli.Command{
	...
	Flags: append([]cli.Flag{
		...
		cli.StringFlag{
			Name:  "icon",
			Usage: "Path to a .svg or .png icon to upload after deploy, single-descriptor files only (content-type, form-fragment, mixin, part and macro kinds)",
		},
		common.FORCE_FLAG,
	...
	Action: func(c *cli.Context) error {
		...
		icon := strings.TrimSpace(c.String("icon"))
		if icon != "" {
			// validate before the deploy so a bad icon does not leave the
			// descriptor deployed without one
			if err := checkIcon(k, icon); err != nil {
		...
		if icon != "" {
			if err := setIcon(c, k, namespace, name, icon); err != nil {
				fmt.Fprintf(os.Stderr, "Schema %s '%s' %s, but icon upload failed: %s\n", k.name, target, verb, err)
				os.Exit(1)
			}
			fmt.Fprintf(os.Stderr, "Schema %s '%s' %s, icon set\n", k.name, target, verb)
		} else {
...
func deployMulti(c *cli.Context, file string, docs []*yaml.Node) error {
	...
	if strings.TrimSpace(c.String("icon")) != "" {
		fmt.Fprintln(os.Stderr, "--icon can not be used with a multi-descriptor file: use the 'icon' field of a document.")
		os.Exit(1)
	}
	...
	type docResult struct {
		...
		IconError string      `json:"iconError,omitempty"`
	...
	for i, root := range docs {
		...
		if err == nil {
			err = resolveEnvelopePaths(env, baseDir)
		}
		var k *kind
		if err == nil {
		...
		if err == nil && env.Icon != "" {
			// validate before the deploy so a bad icon fails the whole
			// document instead of leaving the descriptor deployed without one
			if iconErr := checkIcon(k, env.Icon); iconErr != nil {
				err = errors.Errorf("document %d: %v", env.index, iconErr)
			}
		}
		if err != nil {
			failed++
		...
		if env.Icon != "" {
			if err := setIcon(c, k, namespace, name, env.Icon); err != nil {
				failed++
				fmt.Fprintf(os.Stderr, "Schema %s '%s' %s, but icon upload failed: %s\n", k.name, target, verb, err)
				results = append(results, docResult{Kind: k.name, Target: target, Status: verb, Icon: "failed", IconError: err.Error(), Result: result})
				continue
			}
			iconsSet++
			fmt.Fprintf(os.Stderr, "Schema %s '%s' %s, icon set\n", k.name, target, verb)
			results = append(results, docResult{Kind: k.name, Target: target, Status: verb, Icon: "set", Result: result})
			continue
		}
	...
	if iconsSet > 0 {
		fmt.Fprintf(os.Stderr, ", %d icons set", iconsSet)
	}
	if failed > 0 {
