...
// with a top-level 'resource' or 'resourcePath' key holding a string.
// Descriptors themselves never have one, which makes it a reliable format
// discriminator
func isEnvelopeNode(root *yaml.Node) bool {
	...
	for i := 0; i+1 < len(root.Content); i += 2 {
		...
		if (key.Value == "resource" || key.Value == "resourcePath") && value.Kind == yaml.ScalarNode && value.Tag == "!!str" {
			return true
		}
	}
	return false
...
var resourceFieldRegex = regexp.MustCompile(`(?m)^(resource|resourcePath):`)
...
func decodeEnvelope(root *yaml.Node, index int) (*envelope, error) {
	...
	for i := 0; i+1 < len(root.Content); i += 2 {
		if key := root.Content[i].Value; key != "name" && key != "kind" && key != "resource" && key != "resourcePath" && key != "icon" {
			return nil, errors.Errorf("document %d has an unknown field '%s': allowed fields are name, kind, resource, resourcePath and icon", index, key)
		}
	}
	...
	env.ResourcePath = strings.TrimSpace(env.ResourcePath)
	env.Icon = strings.TrimSpace(env.Icon)
	hasResource := strings.TrimSpace(env.Resource) != ""
	if hasResource == (env.ResourcePath != "") {
		return nil, errors.Errorf("document %d must have exactly one of the 'resource' and 'resourcePath' fields", index)
	}
	return &env, nil
}

// resolveEnvelopePaths resolves the path fields of an envelope against the
// directory of the deploy file and reads the resource file of 'resourcePath'
// documents into Resource, so later stages need not care how the resource was
// given. Absolute paths are used as-is
func resolveEnvelopePaths(env *envelope, baseDir string) error {
	...
	if env.ResourcePath != "" {
		path := resolve(env.ResourcePath)
		content, err := os.ReadFile(path)
		if err != nil {
			return errors.Errorf("document %d: could not read resource file '%s': %v", env.index, path, err)
		}
		env.Resource = string(content)
	}
	env.Icon = resolve(env.Icon)
	return nil
}
